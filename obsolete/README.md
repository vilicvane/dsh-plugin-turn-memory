...
writes the turn's raw transcript into a temporary JSONL draft file and
spawns a fork of the main session — a subagent whose context replays the
completed turns verbatim, including the turn being summarized. The draft has
one JSON object per ORIGINAL surface node — {"seq":N,"kind":"user|assistant|
tool|context","content":"..."} — the node seq is the line id. user and
assistant lines (real user input and user-facing assistant text) are the
final record; tool lines (tool results) and context lines (injected runtime
snapshots, skill catalogs, system reminders) are compressible — the fork
replaces their content with one flowing line, or passes empty content to
drop the node. It works from its context with draft_replace_segment
(targeting lines by their node seq, never reproducing old content),
draft_read_segment and draft_grep for partial lookups, replies DONE, and the
plugin verifies the shape (same line sequence, user/assistant content
byte-for-byte), reads the file back, and lands the summary as an N -> M
replacement: each kept line becomes a same-role replacement event (user
lines -> user/message, assistant lines -> assistant/message, kept tool
lines -> a 1:1 tool/result content rewrite), consecutive same-role nodes
merge, and emptied nodes fold into a neighbour's coverage — so the
conversation's role structure survives on the surface and the message count
drops to the number of kept runs. Assistant replacement messages carry an
id, role, and the span's model source (the harness validates this shape
when the session loads) plus a tool-call block for each kept tool line that
directly follows them, so the provider's tool pairing holds in the next
request; tool lines that cannot be paired fold into a neighbour. The
replacement span starts right after
the turn's own starting user message — that message stays verbatim on the
surface, so the checkpoint never repeats it, and steer messages inside the
span stay verbatim as user/message nodes. The newest user message always
...
A user-initiated turn waits at its boundary for every in-flight summary
fork to settle before its first request is assembled (no wall-clock limit —
the fork settles on its own and a failed fork releases the turn with the
span still raw), so the replacement always lands at the turn boundary and
each turn's request prefix stays stable. There is no in-turn compaction. The compact_turn tool only takes a turn
...
never mix. The checkpoint text carries no turn number: the runtime context
shows the current turn, and older turns are addressed by counting the
verbatim starting user messages on the surface (one per turn, in order).

## Summary format

Each checkpoint is a single user message whose body is a sequence of role
tags in the order things happened — no wrapper of any kind (the checkpoint
is the bare tag sequence; turn identity lives only in the source marker):

    <user-steer:1>
    steering message, verbatim
    </user-steer:1>
    <working:2>
    process between the messages, compressed in place
    </working:2>
    <assistant:3>
    user-facing output, verbatim
    </assistant:3>

    Segment ids are unique, ascending, and stable; every opening and closing
    tag carries its id and occupies its own line exactly. The fork replaces
    a segment by id (draft_replace_segment) without reproducing old content,
    and the parser matches whole tag lines only, so inline tag examples
    inside content never parse as segments.
...
- The checkpoint keeps the original order with no wrapper of any kind: the
  turn's starting user
...
- The replacement is a user/message with the turn-memory source marker
  (turn number, summary id, scope whole-turn). No custom
