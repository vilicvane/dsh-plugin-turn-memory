...
At each multiple of `turnContinuation.reminderIntervalNodes` reached by one root turn, its dynamic runtime context gives the model a short direct action notice to stop, prepare a handoff, and call `continue_after_turn_compression`. The notice and automatic follow-up use ordinary text without XML wrapper tags; structured tags remain only where compression protocols need explicit data boundaries or durable references. The notice remains plugin context rather than a fabricated durable assistant message. The model may keep the current turn only when the whole task will finish in the next few actions; an in-progress atomic mutation is finished before handing off. The counter includes append-origin model-visible messages in the open turn and excludes replaceable runtime snapshots. The notice includes the current node count, estimated token size of the entire canonical context, and next reminder milestone. Durable runtime snapshots remember which milestones were already shown, including across a restart, and the recovery parser still recognizes older tagged snapshots. The successful durable native tool pair or Code Mode sub-dispatch records completed work, current state, unresolved dependencies, and exact next work, then concludes the turn. The plugin first lands and flushes normal completed-turn compression, then submits a direct automatic plugin-sourced follow-up as a separate ordinary turn. If compression has not succeeded, continuation does not start; cold-resume recovery can finish the missing compression and dispatch the still-pending request without duplicating an already inserted follow-up.
