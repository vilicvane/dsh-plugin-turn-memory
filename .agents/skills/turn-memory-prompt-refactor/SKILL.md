...
- Keep raw reasoning intact until ordinary completed-turn compression begins. Long reasoning blocks may have small-model thought hints, but those hints are advisory recall indexes rather than summaries or authoritative evidence: the main worker decides what matters from the inherited original reasoning and verifies details there. Every original assistant node carrying reasoning must be rewritten before landing so raw thoughts do not remain on the compressed surface; hints themselves never become transcript nodes.
- Judge compression by avoided rework rather than by the smallest output. Reasoning may be the only place where an implementation-ready architecture, interface, algorithm, formula, invariant, edge case, validation plan, rejected alternative, or exact continuation state was established. Transfer that operational payload into ordinary assistant memory at enough fidelity that a later capable agent can continue without repeating substantial investigation or design; compress repeated self-talk and mechanical drafting instead of collapsing the work product to topic labels.
